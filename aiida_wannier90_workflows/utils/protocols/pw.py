    ...
    def check_pseudos(self, modifier_name=None, pseudo_data=None):
        ...
        if modifier_name == 'custom':
            if pseudo_data is None:
                raise ValueError(
                    "You chose 'custom' as modifier_name, but did not provide a " 'pseudo_data!')
        else:
        ...
        for element, this_pseudo_data in pseudo_data.items():
            ...
            builder.append(UpfData, filters={'attributes.md5': md5}, project=[
                           'uuid', 'attributes.element'])
            res = builder.all()
            if len(res) >= 1:
