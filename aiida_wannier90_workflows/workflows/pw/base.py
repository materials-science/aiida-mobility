...
def validate_pseudo_family(value, _):
    ...
    if value:
        ...
        warnings.warn(
            '`pseudo_family` is deprecated, use `pw.pseudos` instead.', AiidaDeprecationWarning)
...
class PwBaseWorkChain(ProtocolMixin, BaseRestartWorkChain):
    ...
    def get_builder_from_protocol(
        ...
        **_
        ...
        try:
            ...
            pseudo_family = orm.QueryBuilder().append(
                pseudo_set, filters={'label': pseudo_family}).one()[0]
        ...
        parameters['CONTROL']['etot_conv_thr'] = natoms * \
            meta_parameters['etot_conv_thr_per_atom']
        parameters['ELECTRONS']['conv_thr'] = natoms * \
            meta_parameters['conv_thr_per_atom']
    ...
    def validate_parameters(self):
        ...
        ) if 'settings' in self.ctx.inputs else {}
    ...
    def validate_kpoints(self):
        ...
        try:
            ...
            kpoints = create_kpoints_from_distance(
                **inputs)  # pylint: disable=unexpected-keyword-arg
    ...
    def validate_resources(self):
        ...
        if 'automatic_parallelization' not in self.inputs:
            num_machines = self.ctx.inputs.metadata.options.get(
                'resources', {}).get('num_machines', None)
    ...
    def validate_init_inputs(self):
        ...
        expected_keys = ['max_wallclock_seconds',
                         'target_time_seconds', 'max_num_machines']
    ...
    def report_error_handled(self, calculation, action):
        ...
        """
        arguments = [calculation.process_label, calculation.pk,
    ...
    def sanity_check_insufficient_bands(self, calculation):
        ...
        try:
            ...
            self.report(
                '{}<{}> does not have `output_band` output, skipping sanity check.'.format(*args))
            return
    ...
    def handle_out_of_walltime(self, calculation):
        ...
        try:
            ...
            self.ctx.restart_calc = None
            self.report_error_handled(
                calculation, 'out of walltime: structure changed so restarting from scratch')
    ...
    def handle_relax_recoverable_electronic_convergence_error(self, calculation):
        ...
        self.ctx.restart_calc = None
        self.ctx.inputs.parameters.setdefault(
            'ELECTRONS', {})['mixing_beta'] = mixing_beta_new
        self.ctx.inputs.structure = calculation.outputs.output_structure
