...
def plot_bands(meta, path='', name='pw_bands', wannier_outputs=None, save_name='bands'):
    ...
    ax.hlines(fermi_energy, xmin, xmax, colors='r',
              linestyle='dashed', label='fermi energy')
    ...
    for ilabel in range(len(labels)):
        ...
        ax.vlines(xbar, ymin, ymax, colors='black',
                  linestyle='solid', linewidths=(0.1,), alpha=0.5)
        xbars.append(xbar)
    for i in range(bands.shape[1]):
        ...
        if wannier_outputs is not None:
            ...
            wannier_kxcoords = wannier_seekpath_parameters['explicit_kpoints_linearcoord']
...
def plot_bands_json(wannier, pw_bands):
    ...
    for pos in w_paths:
        if block_index > 0 and w_paths[block_index - 1]['length'] == 1:
            ...
            ax.vlines(label_x, ymin, ymax, colors='black',
                      linestyle='solid', linewidths=(0.1,), alpha=0.5)
        w_x.extend(pos['x'])
        if block_index == 0:
            w_y = pos['values']
    ...
    ax.set_title('Electronic bands structure - %s' %
                 (name))
    ax.set_ylabel(r'Electronic bands ($eV$)')
    ax.hlines(fermi_energy, xmin, xmax, colors='r',
              linestyle='dashed', label='fermi energy')
...
def parse_arugments():
    ...
    parser.add_argument('-W', '--wannier',
                        help="aiida pk of Wannier Workflow",)
    parser.add_argument(
        '-B',
        "--pw_bands",
        help="aiida pk of PW Bands Workflow"
    )
    parser.add_argument(
        '-P',
        "--plot",
