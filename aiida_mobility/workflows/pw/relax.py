...
class PwRelaxWorkChain(ProtocolMixin, WorkChain):
    ...
    def inspect_relax(self):
        ...
        try:
            ...
            if (
                self.inputs.base.pw.parameters["CONTROL"]["calculation"]
                == "scf"
            ):
    ...
    def results(self):
        ...
        if (
            "calculation" not in self.inputs.base.pw.parameters["CONTROL"]
            or self.inputs.base.pw.parameters["CONTROL"]["calculation"] != "scf"
        ):
            self.out(
                "output_structure",
        ...
        try:
            self.out_many(
                self.exposed_outputs(self.ctx.workchain_scf, PwBaseWorkChain)
            )
        except AttributeError:
            self.out_many(
                self.exposed_outputs(final_relax_workchain, PwBaseWorkChain)
            )
