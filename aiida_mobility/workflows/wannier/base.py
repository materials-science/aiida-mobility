...
class Wannier90BaseWorkChain(BaseRestartWorkChain):
    ...
    def setup(self):
        ...
        self.ctx.kmesh_tol_trails = [
            self._WANNIER90_DEFAULT_KMESH_TOL,
            1e-8,
            1e-4,
        ]
    ...
    @process_handler(
        exit_codes=Wannier90Calculation.exit_codes.ERROR_EXITING_MESSAGE_IN_STDOUT
    )
    def handle_kmesh_tol(self, calc):
        ...
        if is_kmesh_tol:
            ...
            self.report(
                "Not enough bvectors found, previous kmesh_tol: {}, trying a new kmesh_tol: {}".format(
                    current_kmesh_tol, new_kmesh_tol
                )
            )
