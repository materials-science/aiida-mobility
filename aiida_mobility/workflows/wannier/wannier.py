...
from aiida_mobility.utils.scdm import fit_scdm_mu_sigma_aiida
...
from aiida_mobility.workflows.wannier.base import Wannier90BaseWorkChain
...
class Wannier90WorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "scdm_thresholds",
            valid_type=orm.Dict,
            default=lambda: orm.Dict(dict={"sigma_factor": 3}),
        ...
        spec.expose_inputs(
            ...
            namespace_options={
                "help": "Inputs for the `Pw2wannier90Calculation` for the pw2wannier90 calculation."
            },
        )
        spec.expose_inputs(
            ...
            exclude=("structure",),
            namespace_options={
                "help": "Inputs for the `Wannier90Calculation` for the Wannier90 calculation."
            },
        )
    ...
    def run_scf(self):
        ...
        self.report(
            f"scf step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def run_nscf(self):
        ...
        running = self.submit(PwBaseWorkChain, **inputs)
        self.report(
            f"nscf step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def should_run_projwfc(self):
        ...
        parameters = inputs.parameters.get_dict()
        scdm_entanglement = parameters.get("inputpp", {}).get(
            "scdm_entanglement", None
        )
        ...
        if scdm_entanglement == "erfc":
            if scdm_mu is not None and scdm_sigma is not None:
                self.report(
                    "found scdm_mu & scdm_sigma in input, skip projwfc calculation."
                )
                return False
            else:
                self.report(
            ...
            if scdm_mu is None or scdm_sigma is None:
                self.report(
                    "scdm_entanglement is gaussian but scdm_mu or scdm_sigma is empty."
                )
    ...
    def run_projwfc(self):
        ...
        self.report(
            f"projwfc step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def inspect_projwfc(self):
        ...
        self.report(
            f"projwfc {calculation.process_label} successfully finished"
        )
    ...
    def run_wannier90_pp(self):
        ...
        self.report(
            f"wannier90 postproc step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def inspect_wannier90_pp(self):
        ...
        # self.ctx.current_folder = workchain.outputs.remote_folder
        self.report(
            f"wannier90 postproc {workchain.process_label} successfully finished"
        )
    ...
    def run_pw2wannier90(self):
        ...
        self.report(
            f"pw2wannier90 step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def run_wannier90(self):
        ...
        self.report(
            f"wannier90 step - launching {running.process_label}<{running.pk}>"
        )
...
def update_scdm_mu_sigma(parameters, bands, projections, thresholds):
    ...
    mu, sigma = fit_scdm_mu_sigma_aiida(
