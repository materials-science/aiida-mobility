...
class Wannier90WorkChain(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            ...
            default=lambda: orm.Dict(
                dict={"max_projectability": 0.95, "sigma_factor": 3}
            ),
    ...
    def run_nscf(self):
        ...
        try:
            ...
            from aiida_quantumespresso.workflows.functions.create_kpoints_from_distance import (
    ...
    def run_projwfc(self):
        ...
        self.report(
            "projwfc step - launching ProjwfcCalculation<{}>".format(running.pk)
        )
    ...
    def run_wannier90_pp(self):
        ...
        if self.ctx.auto_projections:
            if self.inputs.only_valence:
                ...
                inputs.parameters = update_w90_params_numwann(
                    inputs.parameters, self.ctx.calc_projwfc.outputs.projections
                )
                self.report(
        ...
        self.report(
            "wannier90 postproc step - launching Wannier90Calculation<{}> in postproc mode".format(
                running.pk
            )
        )
    ...
    def inspect_wannier90_pp(self):
        ...
        if not workchain.is_finished_ok:
            self.report(
                "wannier90 postproc Wannier90Calculation failed with exit status {}".format(
                    workchain.exit_status
                )
            )
            return self.exit_codes.ERROR_SUB_PROCESS_FAILED_WANNIER90PP
...
def get_keep_bands(parameters):
    ...
    xb_startzero_set = set(
        [idx - 1 for idx in exclude_bands]
    )  # in Fortran/W90: 1-based; in py: 0-based
    keep_bands = np.array(
        [
            idx
            for idx in range(parameters["num_bands"] + len(exclude_bands))
...
def update_pw2wan_params_mu_sigma(
    ...
    def get_mu_and_sigma_from_projections(
        ...
        projwfc_flat, bands_flat = (
            out_array.flatten(),
            bands.get_bands()[:, keep_bands].flatten(),
        )
        # Sorted by energy
