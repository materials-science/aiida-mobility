...
class Wannier90OpengridWorkChain(Wannier90WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.expose_inputs(
            ...
            namespace_options={"help": "Inputs for the `OpengridCalculation`."},
        )
        ...
        spec.outline(
            ...
            cls.run_scf,
            cls.inspect_scf,
            if_(cls.should_run_nscf)(cls.run_nscf, cls.inspect_nscf),
            cls.run_opengrid,
    ...
    def run_opengrid(self):
        ...
        self.report(
            f"open_grid step - launching {running.process_label}<{running.pk}>"
        )
    ...
    def prepare_wannier90_inputs(self):
        ...
        parameters[
            "mp_grid"
        ] = self.ctx.calc_opengrid.outputs.kpoints_mesh.get_kpoints_mesh()[0]
