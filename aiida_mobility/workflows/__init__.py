...
class BaseRestartWorkChain(WorkChain):
    ...
    def process_class(self) -> Type["Process"]:
        ...
        if self._process_class is None or not issubclass(
            self._process_class, Process
        ):
            raise ValueError(
                "no valid Process class defined for `_process_class` attribute"
            )
    ...
    def setup(self) -> None:
        ...
        overrides = (
            self.inputs.handler_overrides.get_dict()
            if (self.inputs and "handler_overrides" in self.inputs)
            else {}
        )
    ...
    def run_process(self) -> ToContext:
        ...
        try:
            ...
            raise AttributeError(
                "no process input dictionary was defined in `self.ctx.inputs`"
            )

        # Set the `CALL` link label
        unwrapped_inputs.setdefault("metadata", {})[
            "call_link_label"
        ] = f"iteration_{self.ctx.iteration:02d}"

        inputs = self._wrap_bare_dict_inputs(
            self.process_class.spec().inputs, unwrapped_inputs
        )
        ...
        considered_handlers = self.node.get_extra(
            self._considered_handlers_extra, []
        )
        considered_handlers.append([])
        self.node.set_extra(
            self._considered_handlers_extra, considered_handlers
        )

        self.report(
            f"launching {self.ctx.process_name}<{node.pk}> iteration #{self.ctx.iteration}"
        )
    ...
    def inspect_process(
        self,
    ) -> Optional["ExitCode"]:  # pylint: disable=too-many-branches
        ...
        if node.is_excepted:
            return (
                self.exit_codes.ERROR_SUB_PROCESS_EXCEPTED
            )  # pylint: disable=no-member

        if node.is_killed:
            return (
                self.exit_codes.ERROR_SUB_PROCESS_KILLED
            )  # pylint: disable=no-member
        ...
        for handler in sorted(
            self.get_process_handlers(), key=get_priority, reverse=True
        ):
            ...
            if report is not None and not isinstance(
                report, ProcessHandlerReport
            ):
                name = handler.__name__
                raise RuntimeError(
                    f"handler `{name}` returned a value that is not a ProcessHandlerReport"
                )
        ...
        if node.is_failed and not last_report:
            if self.ctx.unhandled_failure:
                ...
                return (
                    self.exit_codes.ERROR_SECOND_CONSECUTIVE_UNHANDLED_FAILURE
                )  # pylint: disable=no-member

            self.ctx.unhandled_failure = True
            self.report(
                "{}<{}> failed and error was not handled, restarting once more".format(
                    *report_args
                )
            )
            return None
    ...
    def results(self) -> Optional["ExitCode"]:
        ...
        if (
            ...
            self.report(
                f"reached the maximum number of iterations {self.ctx.max_iterations}: "
    ...
    @classmethod
    def is_process_handler(
        cls, process_handler_name: Union[str, FunctionType]
    ) -> bool:
    ...
    def get_process_handlers(cls) -> List[FunctionType]:
        return [
            ...
            if cls.is_process_handler(method[1])
        ]
    ...
    def _wrap_bare_dict_inputs(
        self, port_namespace: "PortNamespace", inputs: Dict[str, Any]
    ) -> AttributeDict:
