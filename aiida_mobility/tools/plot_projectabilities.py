...
from aiida_mobility.utils.scdm import erfc_scdm, fit_scdm_mu_sigma_aiida
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--save",
        action="store_true",
        help="save as png instead of showing matplotlib window",
    )
    ...
    if "use_opengrid" in wannier90bandsworkchain.inputs:
        ...
        workchain_name = "Wannier90WorkChain"
    wannier90workchain = findLastNode(
        workchain_name, wannier90bandsworkchain.called
    )

    wannier90calculation = findLastNode(
        "Wannier90Calculation", wannier90workchain.called
    )
    fermi_energy = wannier90calculation.inputs.parameters["fermi_energy"]

    pw2wannier90calculation = findLastNode(
    ...
    projwfccalculation = findLastNode(
        "ProjwfcCalculation", wannier90workchain.called
    )
    ...
    mu_fit, sigma_fit, data = fit_scdm_mu_sigma_aiida(
        proj_bands, projections, {"sigma_factor": 0}, True
    )
    # print(sigma, sigma_fit)
    eps = 1e-6
    assert abs(sigma - sigma_fit) < eps
    sigma_factor = wannier90workchain.inputs.scdm_thresholds.get_dict()[
        "sigma_factor"
    ]
